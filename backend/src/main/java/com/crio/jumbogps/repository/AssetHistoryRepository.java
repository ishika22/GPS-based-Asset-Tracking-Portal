...
public interface AssetHistoryRepository extends JpaRepository<AssetHistory, Integer> {
	...
	@Query("SELECT assets FROM AssetHistory assets WHERE fkAssetId.fkAssetType.pkAssetTypeId = :assetType AND timeOfTracking IN(SELECT MAX(timeOfTracking) FROM AssetHistory GROUP BY fkAssetId)")
	...
	@Query("SELECT assets FROM AssetHistory assets WHERE fkAssetId.fkAssetType.pkAssetTypeId = :assetType AND timeOfTracking >= :startTime and timeOfTracking <= :endTime AND timeOfTracking IN(SELECT MAX(timeOfTracking) FROM AssetHistory GROUP BY fkAssetId)")
	List<AssetHistory> getAssetDetailsByTypeAndTime(@Param("assetType") Integer assetType,@Param("startTime") LocalDateTime startTime,@Param("endTime") LocalDateTime endTime);
